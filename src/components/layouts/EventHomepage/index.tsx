...
import {Event, Organizer, OrganizerStatus, VenueAddress} from "../../../types.ts";
...
const EventHomepage = ({colors, continueButtonText, backgroundType, ...loaderData}: EventHomepageProps) => {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => checkTicketsPosition();
    const handleResize = () => checkTicketsPosition();
  ...
  const styleOverrides = {
    "--homepage-body-background-color": colors?.bodyBackground || event?.settings?.homepage_body_background_color,
  ...
  const backgroundColor = colors?.bodyBackground || event?.settings?.homepage_body_background_color || "#f5f5f5";
  const organizer: Organizer = event.organizer as Organizer;
  ...
  const socialLinks = organizerSocials
    ? Object.entries(organizerSocials)
    ...
    <>
      ...
      <div style={styleOverrides} key={`${event.id}`} className={classes.pageWrapper}>
        ...
        {coverImage && backgroundType === "MIRROR_COVER_IMAGE" && (
          <div className={classes.background} style={{backgroundImage: `url(${coverImage})`}} />
        )}
        {(!coverImage || backgroundType === "COLOR") && (
          <div className={classes.background} style={{backgroundColor: "var(--homepage-body-background-color)"}} />
        )}
        ...
        <div id={"event-homepage"} className={classes.mainContainer}>
            ...
            <div className={classes.sectionContent}>
              {coverImage && (
                <div className={classes.coverWrapper}>
                  <img alt={event?.title} src={coverImage} className={classes.coverImage} />
                </div>
              )}
              <div className={`${classes.sectionContent}`}>
          ...
          {/* Tickets Section (glassy card) */}
          <div className={classes.contentSection} ref={ticketsSectionRef}>
            <div className={`${classes.sectionContent} ${classes.glassCard}`}>
              <SelectProducts
                colors={{
                  background: "transparent", // glass comes from wrapper
              ...
              <div className={`${classes.sectionContent} ${classes.glassCard}`}>
                <div className={classes.organizerInfo}>
                  <div className={classes.organizerHeader}>
                    {organizerLogo && (
                      <img src={organizerLogo} alt={organizer.name} className={classes.organizerLogo} />
                    )}
                    <div className={classes.organizerDetails}>
                      <h3 className={classes.organizerName}>
                        <Anchor href={organizerHomepageUrl(organizer)}>{organizer.name}</Anchor>
                      </h3>
          ...
          {/* Footer Section (optional to glassify — uncomment if you want it also glass) */}
          <div className={classes.contentSection}>
            <div className={`${classes.sectionContent}`}>
                  ...
                  <div className={classes.footerLinks}>
                    <Anchor
                      href={
                        getConfig(
                          "VITE_TOS_URL",
                          "https://iCrawn.Events/terms-of-service?utm_source=event=homepage-footer"
                        ) as string
                      }
                    ...
                    <Anchor
                      href={
                        getConfig(
                          "VITE_PRIVACY_URL",
                          "https://iCrawn.Events/privacy-policy?utm_source=event=homepage-footer"
                        ) as string
                      }
                      className={classes.footerLink}
                    >
