...
export const StripePaymentMethod = ({enabled, setSubmitHandler}: StripePaymentMethodProps) => {
    ...
    if (!enabled) {
        return (
            <CheckoutContent>
                <HomepageInfoMessage
                    message={`Stripe payments are not enabled for this event.`}
                    link={eventHomepagePath(event as Event)}
                    linkText={`Return to event page`}
                />
            </CheckoutContent>
        );
    }
    ...
    if (stripePaymentIntentError && event) {
        return (
            <CheckoutContent>
                <HomepageInfoMessage
                    /* @ts-ignore */
                    message={stripePaymentIntentError.response?.data?.message || `Sorry, something has gone wrong. Please restart the checkout process.`}
