...
const AcceptInvitation = () => {
    ...
    const form = useForm({
        ...
        validate: {
            first_name: hasLength({min: 1, max: 50}, `First name must be between 1 and 50 characters`),
            password: hasLength({min: 8}, `Password must be a minimum  of 8 characters`),
            password_confirmation: matchesField('password', `Passwords are not the same`),
            email: isEmail(`Please check your email is valid`),
            terms: (value) => value === true ? null : `You must agree to the terms and conditions`,
        },
    });
    ...
    useEffect(() => {
        ...
        if (user?.data?.status !== 'INVITED') {
            showSuccess(`You have already accepted this invitation. Please login to continue.`);
            navigate('/auth/login');
        }
    ...
    const handleSubmit = (values: AcceptInvitationRequest) => {
        acceptInvitationMutation.mutate({
            ...
            onSuccess: () => {
                showSuccess(`Welcome aboard! Please login to continue.`);
                navigate('/auth/login');
            },
        ...
        <Card>
            ...
            <form onSubmit={form.onSubmit(handleSubmit)}>
                <fieldset disabled={!isFetched}>
                    ...
                    <InputGroup>
                        <TextInput disabled required {...form.getInputProps('email')} label={`Email`}/>
                    ...
                    <Button color={'var(--hi-pink)'} fullWidth loading={acceptInvitationMutation.isPending}
                            type={'submit'}>{`Accept Invitation`}</Button>
                </fieldset>
            </form>
        </Card>
    )
