    ...
    return form?.values?.prices?.map((price, index) => {
        ...
        const cannotDeleteTitle = (() => {
            if (existingPrice && Number(existingPrice?.quantity_sold) > 0) {
                return `You cannot delete this price tier because there are already products sold for this tier. You can hide it instead.`
            }
            if (form?.values?.prices?.length === 1) {
                return `You must have at least one price tier`
            }
            return '';
        })();
        ...
        return (
            <Card key={`price-${index}`} className={classes.priceTierCard}>
                ...
                <Switch
                    description={`Hiding a product will prevent users from seeing it on the event page.`}
                    {...form.getInputProps(`prices.${index}.is_hidden`, {type: 'checkbox'})}
                    label={`Hide this tier from users`}
                />
...
export const ProductForm = ({form, product}: ProductFormProps) => {
    const productTypeOptions: ItemProps[] = [
        ...
        },
        {
            ...
            label: `General`,
            value: 'GENERAL',
            description: `This is a general product, like a t-shirt or a mug. No ticket will be issued`,
        },
    ];
    ...
    return (
        <>
            <div>
                {Number(product?.quantity_sold) > 0 && (
                    <Alert icon={<IconInfoCircle/>} mb={20} color={'blue'}>
                        {`You cannot change the product type as there are attendees associated with this product.`}
                    </Alert>
                )}
                ...
                {form.errors.product_type && (
                    <Alert title={`Product Type`} mb={20} color={'red'}>
                        {form.errors.product_type}
                    </Alert>
                )}
                ...
                {form.errors.type && (
                    <Alert title={`Product Price Type`} mb={20} color={'red'}>
                        {form.errors.type}
                    </Alert>
                )}
                ...
                <TextInput
                           ...
                           label={`Name`}
                           placeholder={form.values.product_type === 'TICKET' ? `VIP Ticke` : `T-shir`}
                           required/>
                ...
                <Select
                    ...
                    label={<InputLabelWithHelp
                        ...
                        helpText={`Categories help you organize your products. This title will be displayed on the public event page.`}
                    />}
            ...
            {form.values.type === ProductPriceType.Tiered && (
                <Fieldset legend={`Price tiers`} mt={20} mb={20}>
            ...
            <Anchor display={'block'} ml={5} mb={20} variant={'subtle'} onClick={toggle}>
                {opened ? `Hide` : `Show`} {`Additional Options`}
            </Anchor>

            <Collapse in={opened}>
                <Fieldset legend={`Additional Options`} mb={20}>
                    <MultiSelect
                        ...
                        description={(
                            <span>
                        {`The taxes and fees to apply to this product. You can create new taxes and fees on the`}{'  '}
                                <NavLink
                                    target={'_blank'}
                                    to={'/account/taxes-and-fees'}>{`Taxes and Fees`}</NavLink> {`page.`}
                                {' '}
                    </span>
                        )}
                        data={[{
                    ...
                    {(form.values.type === ProductPriceType.Free && !!form.values.tax_and_fee_ids?.length) && (
                        <Alert mb={20}>
                            <p>
                                {`You have taxes and fees added to a Free Product. Would you like to remove them?`}
                            </p>
                            <Button onClick={removeTaxesAndFees} size={'xs'}>{`Yes, remove them`}</Button>
                        </Alert>
                    )}

                    <InputGroup>
                        <NumberInput {...form.getInputProps('min_per_order')} label={`Minimum Per Order`}
                                     placeholder="5"/>
                        <NumberInput {...form.getInputProps('max_per_order')} label={`Maximum Per Order`}
                                     placeholder="5"/>
                    </InputGroup>
                    ...
                    <Switch mt={15} {...form.getInputProps('hide_before_sale_start_date', {type: 'checkbox'})}
                            label={`Hide product before sale start date`}/>
                    <Switch mt={20} {...form.getInputProps('hide_after_sale_end_date', {type: 'checkbox'})}
                            label={`Hide product after sale end date`}/>
                    <Switch mt={20} {...form.getInputProps('start_collapsed', {type: 'checkbox'})}
                            label={`Collapse this product when the event page is initially loaded`}/>
                    <Switch mt={20} {...form.getInputProps('show_quantity_remaining', {type: 'checkbox'})}
                            label={`Show available product quantity`}/>
                    <Switch mt={20} {...form.getInputProps('hide_when_sold_out', {type: 'checkbox'})}
                            label={`Hide product when sold ou`}/>
                    <Switch
                        description={<>{`You can create a promo code which targets this product on the`} <NavLink
                            target={'_blank'}
                            to={'../promo-codes'}>{`Promo Code page`}</NavLink></>}
                        mt={20}
                        {...form.getInputProps('is_hidden_without_promo_code', {type: 'checkbox'})}
                        label={`Hide product unless user has applicable promo code`}
                    />
                    <Switch
                        description={`This overrides all visibility settings and will hide the product from all customers.`}
                        {...form.getInputProps(`is_hidden`, {type: 'checkbox'})}
                        label={`Hide this product from customers`}
                    />
                </Fieldset>
            </Collapse>
        </>
