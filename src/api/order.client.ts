...
export const orderClientPublic = {
    initializePaystackPayment: async (eventId: number, orderShortId: string, email: string, amount: number) => {
        const response = await publicApi.post<{ authorization_url: string, reference: string }>(
            `events/${eventId}/order/${orderShortId}/paystack/initialize`,
            { email, amount, event_id: eventId, order_short_id: orderShortId }
        );
        return response.data;
    },
